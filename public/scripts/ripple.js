(function () {
  ...
  var FRAG = `#version 300 es
    ...
    void main() {
      ...
      // Three directional waves (ported from original ripple.vert)
      float h = 0.0;
  ...
  var vs = createShader(gl.VERTEX_SHADER, VERT);
  var fs = createShader(gl.FRAGMENT_SHADER, FRAG);
  if (!vs || !fs) return;
  ...
  var uDark = gl.getUniformLocation(program, 'u_dark');
  var uRippleCount = gl.getUniformLocation(program, 'u_ripple_count');

  var uRippleCenters = [];
  var uRippleStarts = [];
  var uRippleMags = [];
  for (var i = 0; i < 10; i++) {
    uRippleCenters[i] = gl.getUniformLocation(program, 'u_ripple_center[' + i + ']');
    uRippleStarts[i] = gl.getUniformLocation(program, 'u_ripple_start[' + i + ']');
    uRippleMags[i] = gl.getUniformLocation(program, 'u_ripple_mag[' + i + ']');
  }
  ...
  function addRipple(x, y) {
    ...
    var ux = x / window.innerWidth;
    var uy = 1.0 - y / window.innerHeight; // flip Y
  ...
  // Listen on document for clicks within the hero area
  ...
  function frame() {
    ...
    for (var i = 0; i < ripples.length; i++) {
      gl.uniform2f(uRippleCenters[i], ripples[i].cx, ripples[i].cy);
      gl.uniform1f(uRippleStarts[i], ripples[i].start);
      gl.uniform1f(uRippleMags[i], ripples[i].mag);
    }
